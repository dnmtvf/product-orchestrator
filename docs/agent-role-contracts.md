...
- Claude wrapper rule: the stable user-level dispatcher at `~/.codex/skills/pm/scripts/claude-code-mcp` and the copied compatibility wrappers both auto-sync managed `.claude/agents` in the current repo before invoking `claude -p --agent <resolved-name>`, so the public PM launcher contract can remain generic while Claude uses repo-owned project agents internally.
